...
const createToolTip = ({ options, idList, timeList }: ToolTipPrams) => {
  const { series, dataPointIndex, w } = options;
  const idValue = idList[dataPointIndex];
  const time = timeList[dataPointIndex];
  const colors = w.config.legend.markers.fillColors;
  ...
  <div style="border-radius: 5px; box-shadow: 2px 2px 6px -4px #999; cursor: default; font-size: 14px; border: 1px solid #e3e3e3; background: #fff; overflow: hidden; white-space: nowrap;">
    ...
    <ul>
      ${series.reduce((acc: string, crr: string, idx: number) => {
        if (!!crr.length) {
          return (acc += `
            <li style="display: flex; align-items: center; justify-content: left; padding: 6px 10px 8px;">
